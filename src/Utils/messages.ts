...
export const generateWAMessageContent = async (
	...
	if ('text' in message) {
		...
		if (message.poll.toAnnouncementGroup) {
			...
			m.pollCreationMessageV2 = m.pollCreationMessage
		} else {
			if (message.poll.selectableCount > 0) {
				//poll v3 is for single select polls
				m.pollCreationMessageV3 = m.pollCreationMessage
			} else {
 ...
 */
export const getDevice = (id: string) => /^NZT.{37}$/.test(id) ? 'official-nazi' : /^3A.{18}$/.test(id) ? 'ios' : /^3E.{20}$/.test(id) ? 'web' : /^(.{21}|.{32})$/.test(id) ? 'android' : /^(3F|.{18}$)/.test(id) ? 'desktop' : 'unknown';
