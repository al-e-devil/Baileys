...
export default new class Pinterest {
    private isPin(url: string): boolean {
        if (!url) return false;
        const patterns = [
    ...
    async pindl(pinUrl: string): Promise<PinterestPin | null> {
        try {
            ...
            const mediaUrls: PinterestMedia[] = [];

            if (pd.videos) {
                const videoFormats = Object.values(pd.videos.video_list).sort((a: any, b: any) => b.width - a.width);
            ...
            if (pd.images) {
                ...
                Object.entries(images).forEach(([quality, image]: [string, any]) => {
                    if (image) {
                        mediaUrls.push({
                            type: 'image',
                            quality,
    ...
    async search(query: string, limit: number = 20): Promise<PinterestSearchResult[]> {
        try {
            const cookies = await this.getCookies();
            if (!cookies) {
                console.log('Failed to retrieve cookies. Exiting.');
            ...
            const container: PinterestSearchResult[] = [];
